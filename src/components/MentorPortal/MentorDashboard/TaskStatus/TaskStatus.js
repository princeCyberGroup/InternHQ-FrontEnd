  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/api/v4/dashboard-task-status?mentorId=${userId}`, //&batch=${batch}
        {
          headers: {
            ...
            {task.batchUsers.map((batchUser, index) => (
              <div className="mb-3 batch-accord" key={index}>
                <h2 className="accordion-header" id={`heading-${task.taskId}-${index}`}>
                  <button
                    className="accordion-button batch-accord-btn"
                    type="button"
                    data-bs-toggle="collapse"
                ...
                >
                  <div className="accordion-body">
                    {batchUser.users.map((user, userIndex) => (
                      <div key={user.user_id} className="card associate-mapped-card-task">
                        <div className="row d-flex justify-content-space-between">
                          <div className="col-8 d-flex align-items-center">
                            <div className="row">
                          ...
                          <div className="col-4 task-status">
                            {user.task_done ? (
                              user.task_complete_date ? (
                                <>
                                  <Completed />
                                  <div className="mt-1 frame-id-task">{formatDate(user.task_complete_date)}</div>
                                </>
                              ) : (
                                <>
                                  <Ongoing />
